...
export const boardList = (page)=> dispatch => {
    axios.get(`http://localhost/board/list_react/${page}`).then(res => {
        const action={
            type:BOARD_LIST, //board_list:[]
            payload:res.data
        }
        dispatch(action);
        //reducer => 자동을 모든 데이터를 store에 저장
    })
};
...
export const boardDetail = (no)=> dispatch => {
    axios.get(`http://localhost/board/detail_react/${no}`).then(res => {
        const action={
            type:BOARD_DETAIL,
            payload:res.data
        }
...
export const boardDelete = (no,pwd)=> dispatch => {
    axios.delete(`http://localhost/board/delete_react/${no}/${pwd}`).then(res => {
        const action={
            type:BOARD_DELETE,
            payload:res.data
        }
...
export const boardUpdate = (no)=> dispatch => {
    axios.get(`http://localhost/board/update_react/${no}`).then(res => {
        const action={
            type:BOARD_UPDATE,
            payload:res.data
        }
...
export const boardUpdateOk = (no,updateData)=> dispatch => {
    axios({
        method:'put',
        baseURL:'http://localhost',
...
export const boardRest = ()=> dispatch => {
    const action={
        type:RESET,
        payload:{}
    }
